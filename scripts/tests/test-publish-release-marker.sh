...
{"schema_version":1,"upstream_commit":"$commit","codex_version":"$CODEX_VERSION","claude_code_version":"$CLAUDE_CODE_VERSION","created_at":"2026-08-09T20:00:00Z"}
JSON
...
jq -e --arg c "$commit" --arg cdx "$CODEX_VERSION" --arg cld "$CLAUDE_CODE_VERSION" \
  '.upstream_commit == $c and .codex_version == $cdx and .claude_code_version == $cld' \
  "$caught" >/dev/null || {
