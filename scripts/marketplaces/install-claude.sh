...
git config --global url."https://github.com/".insteadOf git@github.com: || die 'failed to configure git URL rewrite (https)'
git config --global --add url."https://github.com/".insteadOf ssh://git@github.com/ || die 'failed to configure git URL rewrite (ssh)'

index=0
while IFS= read -r spec; do
  url=$(jq -er '.url' <<<"$spec") || die 'marketplace spec is missing url'
  ref=$(jq -er '.ref' <<<"$spec") || die 'marketplace spec is missing ref'
  path=$(jq -er '.path' <<<"$spec") || die 'marketplace spec is missing path'
  repository=${url#https://github.com/}
  repository=${repository%.git}
  source_dir="/opt/claude-marketplaces/$index"
  index=$((index + 1))

  git clone --no-checkout --no-single-branch -- "$url" "$source_dir" || die "failed to clone $url"
  git -C "$source_dir" checkout --detach "$ref" || die "failed to check out $ref in $source_dir"
  manifest="$source_dir/$path/.claude-plugin/marketplace.json"
  test -f "$manifest" || die "claude marketplace manifest is missing: $manifest"
  marketplace=$(jq -er '.name' "$manifest") || die "failed to read marketplace name from $manifest"
  ...
  claude plugin marketplace add "$repository" || die "failed to add marketplace: $repository"
  ...
  git -C "$cache_dir" fetch --depth=1 origin "$ref" || die "failed to fetch $ref into marketplace cache: $marketplace"
  git -C "$cache_dir" checkout --detach "$ref" || die "failed to check out $ref in marketplace cache: $marketplace"
  ...
  for plugin in "${selected_plugins[@]}"; do
    claude plugin install "${plugin}@${marketplace}" --scope user || die "failed to install plugin ${plugin}@${marketplace}"
  done
