function claude --description 'Run Claude Code in an ephemeral Microsandbox VM'
    ...
    if not msb volume list --quiet | string match -qx claude-home
        msb volume create --kind disk --size 10G claude-home; or return $status
    end

    # 10G is the persistent-home disk quota; Microsandbox currently has no per-host-bind quota.
    command msb run --tty --pull never --user node --net public --root-disk 20G \
