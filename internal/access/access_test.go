...
func TestLoadValid(t *testing.T) {
	...
	if p.SchemaVersion != 1 || p.Host != "home1.lan.example" || p.Port != 22 || p.User != "claude" || len(p.HostKeys) != 1 {
		t.Fatalf("profile = %+v", p)
	}
	if got, want := p.NetRule(), "allow@home1.lan.example:tcp:22"; got != want {
...
func testProfile() *Profile {
	return &Profile{
		...
		Port:          22,
		User:          "claude",
		HostKeys:      []string{"home1.lan.example ssh-ed25519 AAAA"},
	}
}
...
func TestRenderKnownHostsBareKeyLine(t *testing.T) {
	...
	want := "# Generated by ai-sandbox from an access profile; edits are overwritten.\n" +
		"home2.lan.example ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPEqqWmcHasScOzNO2MFtiFY/x1M1WwoTHHS/wb7jISq\n"
...
func TestRenderKnownHostsSelector(t *testing.T) {
	...
	t.Run("port 22 renders the bare host", func(t *testing.T) {
		p := &Profile{SchemaVersion: 1, Host: "home1.lan.example", Port: 22, User: "claude", HostKeys: []string{key}}
		want := "# Generated by ai-sandbox from an access profile; edits are overwritten.\n" +
			"home1.lan.example ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPEqqWmcHasScOzNO2MFtiFY/x1M1WwoTHHS/wb7jISq\n"
		if got := RenderKnownHosts(p); got != want {
			t.Errorf("RenderKnownHosts = %q, want %q", got, want)
		}
	})
	...
	t.Run("port 2222 renders [host]:port", func(t *testing.T) {
		p := &Profile{SchemaVersion: 1, Host: "home1.lan.example", Port: 2222, User: "claude", HostKeys: []string{key}}
		want := "# Generated by ai-sandbox from an access profile; edits are overwritten.\n" +
			"[home1.lan.example]:2222 ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPEqqWmcHasScOzNO2MFtiFY/x1M1WwoTHHS/wb7jISq\n"
		if got := RenderKnownHosts(p); got != want {
			t.Errorf("RenderKnownHosts = %q, want %q", got, want)
		}
	})
...
func TestLoadRejectsBadHostKeys(t *testing.T) {
	cases := []struct {
		...
		{"wildcard pattern", "*.lan.example ssh-ed25519 AAAA", 22, "unsupported host pattern"},
		{"negated pattern", "!home1.lan.example ssh-ed25519 AAAA", 22, "unsupported host pattern"},
		{"comma-separated patterns", "home1.lan.example,other ssh-ed25519 AAAA", 22, "unsupported host pattern"},
		...
		{"mismatched non-default selector", "home1.lan.example ssh-ed25519 AAAA", 2222, "does not match the destination selector"},
		{"unknown algorithm", "home1.lan.example ssh-weird AAAA", 22, "unsupported key algorithm"},
		{"malformed key material", "home1.lan.example ssh-ed25519 not!base64!!", 22, "malformed key material"},
		{"too few fields", "ssh-ed25519", 22, "not a known_hosts line"},
		{"too many fields", "home1.lan.example ssh-ed25519 AAAA extra", 22, "not a known_hosts line"},
	}
	for _, c := range cases {
		doc := fmt.Sprintf(`{"schema_version":1,"host":"home1.lan.example","port":%d,"user":"u","host_keys":[%q]}`, c.port, c.hostKey)
		configDir := t.TempDir()
...
func TestLoadAcceptsFullAndBareHostKeysOnNonDefaultPort(t *testing.T) {
	for _, hc := range []string{
		...
		doc := fmt.Sprintf(`{"schema_version":1,"host":"home1.lan.example","port":2222,"user":"u","host_keys":[%q]}`, hc)
		configDir := t.TempDir()
		writeProfile(t, configDir, "homelab", doc)
		if _, err := Load(configDir, "homelab"); err != nil {
...
func TestMaterialize(t *testing.T) {
	...
	if !strings.HasSuffix(string(kh), "home1.lan.example ssh-ed25519 AAAA\n") {
